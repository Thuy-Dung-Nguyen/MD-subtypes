...
The UKB phenotype data is stored in 2 mother files named ukb21472.tab (date 180427) and ukb22140.tab (date 180524) in a server. 

# Code template to extract variables
```{bash}
# Build a list for field names
var_list=(eid <put variables here>) # this var_list is changed corresponding to each child dataset extracted
index_array=()
for i in ${var_list[*]}
    do
        index_array+=($(head -1 /<path>/mother_file_name.tab |tr -s "\t" "\n" | cat -n |grep -n $i |cut -d: -f1))
    done
...
cut -f $index_str /<path>/mother_file1.tab > /<path>/child_file_name.tab
```

# Variable list for child dataset
Below, we listed the mother file name and the child file name together with variable list. Child dataset were mentioned in our code to derive variable, Derive_MD_case_control.Rmd and Derive_subtype.Rmd

## 1. Mother dataset ukb21472.tab (date 180427) 
...
## 2. Mother dataset ukb22140.tab (date 180524)
